...
import { AddRole, DeleteRole, getAllData } from "../store";
...
const RoleCards = () => {
  ...
  const onSubmit = async (data) => {
    if (data.roleName?.length) {
      const res = await dispatch(AddRole({ name: data?.roleName }));
      if (res?.payload?.status) {
        ...
        setErrors({
          roleName: {
            type: "manual",
            value: res?.payload?.data?.message,
          },
        });
      }
    } else {
  ...
  const onSubmitDelete = (e) => {
    e.preventDefault();
    dispatch(DeleteRole(deleteItem.id));
